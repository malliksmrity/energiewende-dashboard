...
def preprocess_batch(nlp, texts, batch_size=50):
    """Process many texts in one spaCy pipe call — far faster than calling
    nlp(text) in a per-row .apply() loop, which re-pays pipeline overhead
    on every single document."""
    results = []
    for doc in nlp.pipe((t.lower() for t in texts), batch_size=batch_size):
        results.append([
            token.lemma_ for token in doc
            if token.is_alpha and not token.is_stop
            and token.pos_ != "PROPN"
...
def fetch_germaparl(periods=("17", "18", "19"), step=30, max_total=200):
    ...
    for period in periods:
        ...
        for fname in files:
            ...
            try:
                ...
                continue
    return pd.DataFrame(all_records[:max_total])
...
def train_classifier(df_raw):
    ...
    df["tokens"] = preprocess_batch(nlp, df["text"].tolist())
...
st.caption(
    "First load trains the classifier from scratch and can take 1-3 minutes — "
    "the page is working even if nothing visibly changes for a bit. "
    "Later visits are instant thanks to caching."
)
with st.spinner("Loading pipeline — first load can take a few minutes…"):
    nlp = load_nlp()
